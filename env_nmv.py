...
class NMVDLSchedulingEnv(gym.Env):
    ...
    def _step_self_test(self) -> None:
        ...
        for task_id1 in range(self.num_tasks + 1):
            for task_id2 in range(self.num_tasks + 1):
                action = np.array([task_id1, task_id2])
    ...
    def __init__(
        ...
        super(NMVDLSchedulingEnv, self).__init__()
        ...
        for model_name in self.model_name_set:
            self.models[(model_name, 0)] = load_model(
                model_name,
                1,
                "cifar10" if "alexnet" not in model_name else "GTSRB",
            )
            self.models[(model_name, 0)].to("cuda:0")
    ...
    def _extract_variant_info(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for _, row in self.model_info.iterrows():
            ...
            if model_name not in runtime_dict:
                runtime_dict[model_name] = [None]
                accuracy_dict[model_name] = [None]
            runtime_dict[model_name][0] = runtime
            accuracy_dict[model_name][0] = accuracy
    ...
    def execute_task(
        ...
        model: nn.Module = self.models[(task["model"], 0)]
    ...
    def valid_action_mask(self) -> np.ndarray:
        ...
        action_mask = np.array([task_mask1, task_mask2], dtype=np.bool_)
        return action_mask
...
def train_TRPO(time_step_of_training: int, test_kind: str) -> None:
    ...
    for workload in worloads:
        env: NMVDLSchedulingEnv = NMVDLSchedulingEnv(
